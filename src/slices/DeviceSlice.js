...
//     state.Devices = action.payload.map(device => device.device_id);

//     console.log("devices redux", state.Devices);
// })
...
export const createDevice = createAsyncThunk(
  ...
  async ({ deviceId, venueId, conditions = [] }, { rejectWithValue }) => {
    try {
      ...
      const data = await res.json();
      if (!res.ok) return rejectWithValue(data.message || "Failed to create device");
